...
ASSET_FILE_VALID_EXTENSIONS = ['mp4', 'ogv', 'webm', 'avi', 'mov', 'ogg',
                               'mp3', 'wav', 'm4a']
...
class AssetFile(models.Model):
    ...
    content = models.FileField(
        validators=[FileExtensionValidator(
            allowed_extensions=ASSET_FILE_VALID_EXTENSIONS)],
...
class AssetFilePart(models.Model):
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    asset_file = models.ForeignKey('AssetFile',
                                   related_name='asset_file_parts',
                                   on_delete=models.CASCADE)
    content = models.FileField(
        validators=[FileExtensionValidator(allowed_extensions=['mp3'])],
        upload_to=asset_file__part_upload_path)
